...
async function UsulanList() {
  ...
  // Aturan hapus berurutan: hanya tahap TERAKHIR yang ada (paling kanan pada
  // urutan Kebutuhan → Indikatif → Anggaran → Alokasi) yang boleh dihapus, dan
  // hanya bila masih Draft.
  const lastTahapIdx = rows.reduce((mx: number, u: any) => {
    const i = TAHAP_ORDER.indexOf(u.tahap_pagu as TahapPagu);
    return i > mx ? i : mx;
  }, -1);
  ...
  return (
    <Card className="divide-y divide-border">
      {(list ?? []).length === 0 ? (
        ...
        const tahapIdx = TAHAP_ORDER.indexOf(u.tahap_pagu as TahapPagu);
        const isDraft = u.status === 'Draft';
        const isLastTahap = tahapIdx === lastTahapIdx;
        // Hanya tahap terakhir & masih Draft yang bisa dihapus (semua peran).
        const canDelete = isDraft && isLastTahap;
        const disabledReason = !isDraft
          ? 'Tahap Final harus dikembalikan ke Draft sebelum dihapus'
          : 'Hapus dahulu tahap setelahnya — penghapusan harus berurutan dari tahap terakhir';
        return (
