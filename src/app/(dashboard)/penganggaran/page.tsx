          ...
          <Link key={u.id} href={`/penganggaran/${u.id}`} className="flex items-center gap-4 p-4 hover:bg-accent">
            <div className="flex-1">
              <div className="font-medium">
                TA {u.tahun_anggaran} — {u.kegiatan?.kode_kegiatan ? `${u.kegiatan.kode_kegiatan} ` : ''}{u.kegiatan?.nama_kegiatan ?? 'Usulan'}
              </div>
              <div className="text-xs text-muted-foreground">
                {u.program?.kode_program ? `${u.program.kode_program} — ${u.program.nama_program}` : ''}
              </div>
