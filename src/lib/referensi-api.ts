...
export async function listMaster(
  ...
  if (q.trim()) query = query.or(`${def.kodeCol}.ilike.%${q}%,${def.namaCol}.ilike.%${q}%`);
  query = query.order(def.kodeCol, { ascending: true }).range(from, from + perPage - 1);
...
export async function listParents(def: MasterDef): Promise<{ id: string; label: string }[]> {
...
export async function createMaster(def: MasterDef, values: Record<string, unknown>): Promise<void> {
  const { error } = await sb().from(def.table).insert(values);
  if (error) throw error;
}

export async function updateMaster(def: MasterDef, id: string, values: Record<string, unknown>): Promise<void> {
...
export async function bulkImport(
  def: MasterDef,
  rows: ParsedRow[],
): Promise<{ inserted: number; skipped: number; failed: number; errors: string[] }> {
  ...
  if (def.parent) {
    const p = def.parent;
    const { data } = await sb().from(p.table).select(`id, ${p.kodeCol}`).limit(5000);
    (data ?? []).forEach((r: Record<string, unknown>) => parentMap.set(String(r[p.kodeCol]), r.id as string));
  }
  ...
  for (const r of valid) {
    ...
    if (def.parent) {
      ...
      if (!pid) { failed++; errors.push(`Induk "${r.parentCode}" tidak ditemukan untuk kode ${r.values[def.kodeCol]}`); continue; }
  ...
  const conflictCols = def.parent ? `${def.parent.fkCol},${def.kodeCol}` : def.kodeCol;
  if (payloads.length) {
    const { error, count } = await sb()
      .from(def.table)
      .upsert(payloads, { onConflict: conflictCols, ignoreDuplicates: true, count: 'exact' });
    if (error) { errors.push(error.message); failed += payloads.length; }
